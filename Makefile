...
build-run-plain:
	docker run -p 127.0.0.1:22:222 --network host --name kali -itd kalilinux/kali-rolling; docker attach kali 
...
image-run-s:
	docker run -it -p 22:2222 --network host --rm --privileged --workdir /usr --name kali-systemd kali /bin/bash
...
docker-p-b:
	...
	docker run -p 22:2222 --network host --rm --privileged --workdir /usr --name kali_p -itd lostcauze7/kali-dockerized:latest; docker attach kali_p
